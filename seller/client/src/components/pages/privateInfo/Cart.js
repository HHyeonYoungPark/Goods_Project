...
function Cart({userId}) {
  ...
  const handleAllCheck = (checked) => {
    if(checked) {
      ...
      cart.forEach((el) => checkedItemsArray.push(el.idx));
  ...
  const handleCheck = (checked, idx) => {
    if (checked) {
      ...
      setCheckItems([...checkedItems, idx]);
    } else {
      // 단일 선택 해제 시 체크된 아이템을 제외한 배열 (필터)
      setCheckItems(checkedItems.filter((el) => el !== idx));
    }
  };
  ...
  */
  ...
  return (
    <div className="cartContent-wrap">
                ...
                //  <CartItem item={item} key={key}/>
                  ...
                  return(
                    <tr key={key}>
                      <td>
                        <input type="checkbox"
                          onChange={(e) => handleCheck(e.target.value, item.idx)}
      ...
      <div className='cartContent-rigt'>
